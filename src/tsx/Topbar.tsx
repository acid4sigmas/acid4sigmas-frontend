...
import { Account, CloudTheme, CloudthemesStatus } from "../types";
  ...
  useEffect(() => {
    ...
    if (token !== null) {
      ...
      const fetchCloudThemeStatus = async () => {
        try {
          const result = await fetch(
            config.api_url + "/api/cloudthemes/status",
            {
              method: "GET",
              headers: {
                Authorization: token,
          ...
          if (response.enabled) {
            console.log("cloud themes are enabled");
            await fetchCloudtheme();
          }
        } catch (e) {
          console.error(e);
        }
      ...
      const fetchCloudtheme = async () => {
        try {
          const result = await fetch(config.api_url + "/api/cloudthemes", {
            method: "GET",
            headers: {
              Authorization: token,
            },
          ...
          const response: CloudTheme = await result.json();
